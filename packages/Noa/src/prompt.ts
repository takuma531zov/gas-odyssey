...
5. 上記いずれでもない、または情報不足の場合：null

### 出力フォーマット:

**レシート・領収書・駐車券等の場合（必ず1件のみ）:**
[
  {
    "店舗名": "店舗名または発行者及び会社名",
    "日付": "MM/DD",
...
- 金額らしき数値が複数ある場合、最も大きい値を合計金額として採用
- 店舗名抽出時は企業名も含めて設定（例：Seria北浦和店、ローソン○○店）
- 店舗名が不明確な場合、住所や電話番号から推測可能なら補完

### レシートの読み取りルール:
- クレジットカード明細のみ、1つのOCRテキスト内に複数取引が含まれるため配列化
- クレジットカード明細の引き落とし日：任意の値で可（システム側で自動計算）
- カード名は常に"三井住友カード"として設定
- 品目例：食料品代、ガソリン代、駐車場代など
