...
import type {
  ExtractorFormAnalysisResult as FormAnalysisResult,
  StructuredFormResult,
  ExtractorGoogleFormResult as GoogleFormResult,
  FormExclusionResult,
} from "../../../common/types";
...
export const hasScriptAndRecaptcha = (html: string): boolean => {
  const hasScript =
    /<script[^>]*>[\s\S]*?<\/script>/gi.test(html) ||
    /<script[^>]*src=[^>]*>/gi.test(html);
...
export const analyzeFormElements = (html: string): FormAnalysisResult => {
  ...
  const formElementCount = FORM_ELEMENT_KEYWORDS.filter((element) =>
  ...
  const foundContactKeywords = CONTACT_KEYWORDS.filter((keyword) =>
    lowerHtml.includes(keyword.toLowerCase()),
  );
...
export const hasSubmitButtonInForm = (formHTML: string): boolean => {
  ...
  return submitButtonPatterns.some((pattern) => {
    ...
    return matches
      ? matches.some((match) => containsSubmitKeyword(match))
      : false;
  });
};
    ...
    if (matches) {
      for (const match of matches) {
        const urlMatch = match.match(
          /(['"])(.*docs\.google\.com\/forms\/d\/[a-zA-Z0-9-_]+\/?[^"'\s\)]*?)\1/,
        );
        if (urlMatch?.[2]) {
          return {
            found: true,
            url: urlMatch[2],
            type: pattern.source.includes("<a")
              ? "direct_link"
              : "iframe_embed",
          };
        }
...
export const findEmbeddedHTMLForm = (html: string): boolean => {
  ...
  while (formMatch) {
    ...
    const excludeResult = shouldExcludeForm(
      formTag,
      formContent,
      html,
      formMatch.index,
    ...
    if (!excludeResult.shouldExclude) {
      const matchingKeywords = EMBEDDED_FORM_CONTACT_FIELD_KEYWORDS.filter(
        (keyword) => formContent.toLowerCase().includes(keyword.toLowerCase()),
      );
 ...
 * フォーム除外判定（純粋関数）
 */
export const shouldExcludeForm = (
  formTag: string,
  formContent: string,
  html: string,
  formIndex: number,
  ...
  if (method === "get") {
    if (
      FORM_EXCLUDE_SEARCH_KEYWORDS.some(
        (keyword) =>
          lowerFormContent.includes(keyword) ||
          action?.toLowerCase().includes(keyword),
      )
    ) {
      return {
        shouldExclude: true,
        reason: "GET method with search keywords",
        priority: "exclude",
      };
    }
  }
  ...
  if (action) {
    if (
      FORM_EXCLUDE_ACTIONS.some((pattern) =>
        action.toLowerCase().includes(pattern),
      )
    ) {
      return {
        shouldExclude: true,
        reason: `Excluded action: ${action}`,
        priority: "exclude",
      };
    }
  }
  ...
  const contextEnd = Math.min(
    html.length,
    formIndex + formContent.length + 500,
  );
  const context = html.substring(contextStart, contextEnd).toLowerCase();
  const foundExcludeKeywords = FORM_EXCLUDE_CONTEXT_KEYWORDS.filter((keyword) =>
    context.includes(keyword),
  );
  if (foundExcludeKeywords.length > 0) {
    return {
      shouldExclude: true,
      reason: `Context keywords: ${foundExcludeKeywords.join(", ")}`,
      priority: "exclude",
    };
  }
  ...
  if (method === "post") {
    ...
    priority = FORM_CONTEXT_CONTACT_KEYWORDS.some((keyword) =>
      context.includes(keyword),
    )
      ? "medium"
      : "low";
  ...
  return {
    shouldExclude: false,
    reason: `Valid form (method: ${method || "unspecified"}, action: ${action || "unspecified"})`,
    priority,
  };
};
 ...
 */
export const validateGoogleFormContent = (
  html: string,
  ...
  const contextEnd = Math.min(
    html.length,
    formUrlIndex + googleFormUrl.length + 1000,
  );
  const context = html.substring(contextStart, contextEnd).toLowerCase();

  if (
    GOOGLE_FORM_EXCLUDE_KEYWORDS.some((keyword) =>
  ...
  if (
    GOOGLE_FORM_CONTACT_KEYWORDS.some((keyword) =>
      context.includes(keyword.toLowerCase()),
    )
  ) {
    return true;
...
export const createFormValidator =
  (validationRules: Array<(html: string) => boolean>) =>
  (html: string): boolean =>
 ...
 */
export const createGoogleFormExtractor =
  (patterns: RegExp[]) =>
