 ...
 * キーワードの重み値を取得（純粋関数）
 */
export const getKeywordWeight: ScoreCalculator = (
  keyword: string,
  location: "url" | "text",
): number => {
    ...
    matches.reduce(
      (acc, keyword) => {
        ...
        return {
          ...
          reasons: [
            ...acc.reasons,
            `${location}_keyword:${keyword}(+${points})`,
          ],
        };
      },
 ...
 * 純度計算用の合成関数
 */
const matchHighPriorityKeywords = createKeywordMatcher(
  HIGH_PRIORITY_CONTACT_KEYWORDS,
);
...
export const calculateContactPurity = (
  ...
  console.log(
    `Calculating contact purity for: URL="${url}", Text="${linkText}", Context="${context}"`,
  );

  // URL内の高優先度キーワードチェック（関数型）
  const urlMatches = matchHighPriorityKeywords(lowerUrl);
  const urlScore = calculateKeywordScore(
    HIGH_PRIORITY_CONTACT_KEYWORDS,
    "url",
  )(urlMatches);
  ...
  const textScore = calculateKeywordScore(
    HIGH_PRIORITY_CONTACT_KEYWORDS,
    "text",
  )(textMatches);
  ...
  // ネガティブパターンチェック（関数型）
  const negativeMatches = NEGATIVE_PATTERNS.filter(
    (negative) => lowerUrl.includes(negative) || lowerText.includes(negative),
  );
  ...
  console.log(
    `  Final purity score: ${score} (reasons: ${reasons.join(", ")})`,
  );
...
export const detectKeywords = (
  html: string,
  keywords: string[] = HIGH_PRIORITY_CONTACT_KEYWORDS,
): KeywordDetectionResult => {
  const lowerHtml = html.toLowerCase();
  const foundKeywords = createKeywordMatcher(keywords)(lowerHtml);

  console.log(
    `Keyword detection: found ${foundKeywords.length} unique keywords: ${foundKeywords.join(",")}`,
  );
...
export const calculateDynamicSiteKeywordScore = (html: string): number => {
  ...
  const totalScore = scoreCalculators.reduce(
    (total, { keywords, multiplier, label }) => {
      const score = keywords.reduce((acc, keyword) => {
        const matches = (
          lowerHtml.match(new RegExp(keyword.toLowerCase(), "g")) || []
        ).length;
        if (matches > 0) {
          console.log(
            `Found ${matches} occurrences of "${keyword}" (${label})`,
          );
        }
